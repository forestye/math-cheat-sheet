...
\begin{mybox}[4. 谱图论 (Spectral Graph)]{headerblue}
    \boxdesc{用线性代数研究图结构}
    ...
    \subt{图的嵌入与表示}
    \begin{itemize}
        \item \textbf{图同构}: 判断两图结构是否相同 (NP问题)。
        \item \textbf{谱半径}: 最大特征值，反映扩散速度。
    \end{itemize}
\end{mybox}
...
\begin{mybox}[5. 网络流 (Network Flow)]{headerblue}
    \boxdesc{管道、运输与瓶颈}
...
\begin{mybox}[ 核心直觉 (Intuition)]{intuitioncolor}
    ...
    \begin{tikzpicture}[scale=0.7, auto, swap]
        ...
        \foreach \source/ \dest /\weight in {b/a/7, c/b/8,d/a/5,d/b/9,
                                             ...
                                             g/e/9,g/f/11}
            \path[draw,gray] (\source) -- node[midway, font=\tiny] {$\weight$} (\dest);
        % 突出一条路径
