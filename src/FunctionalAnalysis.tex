...
\begin{mybox}[3. 算子理论 (Operator Theory)]{headerblue}
    ...
    \subt{自反空间 (Reflexive Space)}
    若典范嵌入 $J: X \to X^{**}$ 是满射 ($X \cong X^{**}$)。
    \textit{例: Hilbert 空间和 $L^p (1<p<\infty)$ 是自反的。}
    \sepline
    ...
    \subt{紧算子 (Compact Operator)}
    将有界集映射为相对紧集 (表现像有限维矩阵)。
    \textit{Fredholm 选择定理: 紧算子谱理论的基础。}
\end{mybox}
...
\begin{mybox}[4. 三大基本定理 (The Big Three)]{headerblue}
    ...
    \subt{4. 闭图像定理 (Closed Graph)}
    若 $T: X \to Y$ 的图像在 $X \times Y$ 中闭，则 $T$ 连续。
\end{mybox}
...
\begin{mybox}[5. 谱理论 (Spectral Theory)]{headerblue}
    ...
    \textbf{预解集}: $\rho(T) = \mathbb{C} \setminus \sigma(T)$ (算子可逆)。
    \begin{itemize}
        \item \textbf{点谱}: 真正的特征值 ($Tx = \lambda x$)。
        \item \textbf{连续谱}: 类似 $Tf(x) = xf(x)$ 这种算子。
        \item \textbf{剩余谱}: 像空间不稠密。
    \end{itemize}
    \sepline
    
    \subt{Fredholm 二择一}
    紧算子扰动 $I-K$ 要么可逆，要么有非零核。
    \sepline
...
\begin{mybox}[8. 高阶应用 (Advanced)]{headerblue}
    ...
    \subt{弱拓扑 (Weak Topology)}
    $x_n \rightharpoonup x \iff \forall f \in X^*, f(x_n) \to f(x)$。
    \textit{Banach-Alaoglu: 对偶单位球是弱*紧的。}
\end{mybox}
