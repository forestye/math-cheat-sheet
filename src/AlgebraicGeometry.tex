...
\begin{mybox}[1. 仿射簇 (Affine Varieties)]{headerblue}
    ...
    \subt{理想 (Ideal) $I(V)$}
    在 $V$ 上全为 0 的多项式集合。
    \textbf{坐标环}: $k[V] = k[x_1, \dots, x_n] / I(V)$。
    \sepline
...
\begin{mybox}[3. 概型 (Schemes)]{headerblue}
    ...
    \subt{态射 (Morphisms)}
    概型间的映射 $f: X \to Y$ 由连续映射和层同态组成。
    \textit{代数几何中的函数不仅是点到点的映射，还拉回函数。}
    \sepline
    
    \subt{纤维积 (Fiber Product)}
    $X \times_S Y$: 概型范畴中的“交集”操作。
\end{mybox}

\begin{mybox}[4. 维数理论 (Dimension)]{headerblue}
    \boxdesc{大小的度量}
    
    \subt{Krull 维数}
    素理想链的最大长度：$P_0 \subsetneq P_1 \subsetneq \dots \subsetneq P_d$。
    \textit{对应几何对象的维数 (点=0, 线=1, 面=2)。}
    \sepline
    
    \subt{切空间 (Tangent Space)}
    在点 $p$ 处的线性逼近。
    $$ T_p(V) = (I(V) / \mathfrak{m}_p^2)^* $$
\end{mybox}

\begin{mybox}[5.  Gröbner 基 (Gröbner Basis)]{headerblue}
    \boxdesc{多项式的“高斯消元”}
...
\begin{mybox}[6. 椭圆曲线 (Elliptic Curves)]{headerblue}
    \boxdesc{代数几何的明珠}
    ...
    \textit{Mordell-Weil 定理: $\mathbb{Q}$ 上的有理点群是有限生成的。}
    \sepline
    
    \subt{黎曼-罗赫定理 (Riemann-Roch)}
    计算具有特定极点的函数空间的维数。
    $$ l(D) - l(K-D) = \deg(D) - g + 1 $$
    \textit{代数几何中最重要的计数公式。}
    \sepline
    ...
    \begin{itemize}
        ...
        \item \textbf{j-invariant}: $j(E)$ 唯一确定同构类。
    \end{itemize}
\end{mybox}

\begin{mybox}[7. 奇点理论 (Singularities)]{headerblue}
    \boxdesc{光滑性的破坏}
    
    \subt{光滑点}
    雅可比矩阵满秩的点。
    \textit{例: $y^2=x^3$ 在 $(0,0)$ 处奇异 (尖点)。}
    \sepline
...
\begin{mybox}[8. Python / SageMath 实战]{headerblue}
    \boxdesc{代码工具箱}
    \begin{itemize}
        \item \textbf{SageMath}: 最强开源数学软件。
        \item \texttt{R.<x,y,z> = PolynomialRing(QQ)}
        \item \texttt{I = R.ideal([x\textasciicircum 2+y\textasciicircum 2-1, x-y])}
