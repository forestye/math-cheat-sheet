...
\begin{mybox}[2. 泛性质 (Universal Property)]{headerblue}
    \boxdesc{通过关系定义对象}
...
\begin{mybox}[4. 函子 (Functor)]{headerblue}
    \boxdesc{范畴间的映射}
    ...
    \subt{应用函子 (Applicative)}
    介于 Functor 和 Monad 之间。
    \textit{允许将包装在上下文中的函数应用到包装在上下文中的值。}
\end{mybox}
...
\begin{mybox}[6. 幺半范畴 (Monoidal)]{headerblue}
    ...
    \subt{闭范畴 (Closed Category)}
    拥有内部 Hom 对象 (指数对象 $B^A$)。
    \textit{对应函数类型 \texttt{A -> B}。}
\end{mybox}
