...
\begin{mybox}[1. 熵与不确定性 (Entropy)]{headerblue}
    ...
    \subt{联合熵与条件熵}
    \begin{itemize}
        \item $H(X,Y) = H(X) + H(Y|X)$ (链式法则)
        \item $H(Y|X)$: 知道 $X$ 后 $Y$ 的不确定性
    \end{itemize}
\end{mybox}
...
\begin{mybox}[3. 信源编码 (Source Coding)]{headerblue}
    ...
    \begin{itemize}
        ...
        \item \textbf{平均码长}: $H(X) \le L < H(X) + 1$。
    \end{itemize}
    \sepline
    ...
    \subt{Lempel-Ziv 编码}
    动态构建字典，适用于未知分布。
    \textit{应用: ZIP, GZIP, PNG。}
    \sepline
    
    \subt{渐进等价性 (AEP)}
    长序列的典型概率 $\approx 2^{-nH(X)}$，大多数序列都是典型序列。
\end{mybox}
...
\begin{mybox}[4. 信道编码 (Channel Coding)]{headerblue}
    ...
    \subt{纠错码}
    \begin{itemize}
        \item \textbf{汉明码}: 单比特纠错。
        \item \textbf{LDPC/Turbo}: 接近香农极限的实用码。
        \item \textbf{Reed-Solomon}: 纠突发错误 (光盘/二维码)。
    \end{itemize}
\end{mybox}
...
\begin{mybox}[6. 机器学习视角 (ML View)]{headerblue}
    ...
    \subt{信息增益 (Information Gain)}
    决策树分裂标准: $IG = H(Y) - H(Y|X)$。
    \textit{选择信息增益最大的特征进行分裂。}
\end{mybox}
...
\begin{mybox}[8. 高阶概念 (Advanced)]{headerblue}
    ...
    深度学习理论：压缩输入 $I(X;T)$，保留输出 $I(T;Y)$。
    \sepline
    
    \subt{科尔莫哥洛夫复杂度}
    描述对象所需的最短程序长度 (熵的算法版本)。
    \textit{不可计算，但可近似估计。}
    \sepline
    
    \subt{数据处理不等式}
    如果 $X \to Y \to Z$ 构成马尔可夫链：
    $$ I(X;Z) \le I(X;Y) $$
    \textit{信息只会减少，不会增加。}
\end{mybox}
...
\begin{mybox}[ 核心直觉 (Intuition)]{intuitioncolor}
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        % 轮廓
        \draw[thick, headerblue] \firstcircle;
        \draw[thick, section2] \secondcircle;
        
        % 外部标签
        \node[headerblue] at (-1.4, 1.4) {$H(X)$};
        \node[section2] at (3.2, 1.4) {$H(Y)$};
    ...
    \begin{itemize}[itemsep=4pt]
        ...
        \item \textbf{相关性即共享}: 
        互信息 $I(X;Y)$ 衡量变量共享比特数。独立则互信息为 0。
        
        \item \textbf{编码即理解}: 
        如果你能把数据压缩得很小，说明你理解了它的规律。压缩率是智能的度量。
    \end{itemize}
