...
\begin{mybox}[5. 机制设计 (Mechanism Design)]{headerblue}
    ...
    \subt{激励兼容 (Incentive Compatible)}
    说真话是每个玩家的最优策略。
    \textbf{Revelation Principle}: 任何机制都等价于某个激励兼容机制。
    \textit{直觉：不需要猜心，直说即可。}
\end{mybox}
...
\begin{mybox}[7. 演化博弈 (Evolutionary)]{headerblue}
    ...
    $$ u(S, S) > u(T, S) $$
    或者 $u(S, S) = u(T, S)$ 且 $u(S, T) > u(T, T)$。
    \textit{应用: 生物学 (鹰鸽博弈)，算法优化。}
    \sepline
    
    \subt{复制动态方程 (Replicator Dynamics)}
    策略比例随时间演化：
...
\begin{mybox}[8. Python / Gambit 实战]{headerblue}
    ...
    \begin{itemize}
        ...
        \item \textbf{Gambit}: 求解静态博弈均衡 (支持命令行)。
        \item \textbf{OR-Tools}: Google 优化库 (线性规划/约束满足)。
    \end{itemize}
\end{mybox}
