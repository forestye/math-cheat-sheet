...
\begin{mybox}[3. 离散傅里叶 (DFT)]{headerblue}
    ...
    $k$: 频率索引，$W_N = e^{-i\frac{2\pi}{N}}$ 为 $N$ 次单位根。
    \sepline
    
    \subt{采样定理 (Nyquist)}
    采样频率必须大于信号最高频率的 2 倍 ($f_s > 2f_{max}$)。
    \sepline
    
    \subt{频谱泄露 (Leakage)}
    信号周期不是 $N$ 的整数倍时，能量会泄露。\textit{解法: 加窗 (Hamming)。}
\end{mybox}
...
\begin{mybox}[2. 核心性质 (Properties)]{headerblue}
    ...
    \subt{其他重要性质}
    \begin{itemize}
        \item \textbf{平移}: $f(t-t_0) \iff e^{-i\omega t_0} F(\omega)$
        \item \textbf{调制}: $f(t)e^{i\omega_0 t} \iff F(\omega - \omega_0)$
        \item \textbf{尺度}: $f(at) \iff \frac{1}{|a|} F(\frac{\omega}{a})$
    \end{itemize}
\end{mybox}
...
\begin{mybox}[8. 高阶技巧 (Advanced)]{headerblue}
    ...
    \subt{数论变换 (NTT)}
    在有限域 $GF(p)$ 上做 FFT。
    \textit{优势: 无浮点误差。应用: 大数乘法库, 密码学。}
    质数 $p$ 需满足 $p = k \cdot 2^n + 1$ 形式。
    \sepline
    
    \subt{压缩感知 (Compressed Sensing)}
    信号在频域稀疏时，可以低于奈奎斯特频率采样并恢复。
    $$ \min \|x\|_1 \quad \text{s.t.} \quad y = \Phi x $$
    \textit{应用: MRI成像加速、单像素相机。}
\end{mybox}
