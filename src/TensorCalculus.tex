...
\begin{mybox}[3. 协变导数 (Covariant Derivative)]{headerblue}
    ...
    \subt{度量相容性}
    $\nabla_\gamma g_{\alpha\beta} = 0$ (度量在平移下不变)。
\end{mybox}

\begin{mybox}[4. 测地线 (Geodesics)]{headerblue}
    \boxdesc{弯曲空间中的直线}
    
    \subt{测地线方程}
    $$ \frac{d^2 x^\mu}{d\tau^2} + \Gamma^\mu_{\nu\lambda} \frac{dx^\nu}{d\tau} \frac{dx^\lambda}{d\tau} = 0 $$
    \textit{物理意义: 自由落体运动的轨迹 (无外力)。}
    \sepline
    
    \subt{平移 (Parallel Transport)}
    向量沿曲线移动时保持“平行” ($\nabla_{\dot{\gamma}} v = 0$)。
    测地线就是切向量沿自身平移的曲线。
\end{mybox}

\begin{mybox}[5. 曲率 (Curvature)]{headerblue}
    \boxdesc{平移一圈回不到原点}
...
\begin{mybox}[6. 对称性 (Symmetries)]{headerblue}
    \boxdesc{守恒律的源头}
    
    \subt{Killing 矢量场 $K$}
    满足 $\nabla_\mu K_\nu + \nabla_\nu K_\mu = 0$。
    \textit{对应度量的等距变换 (如时间平移 $\to$ 能量守恒)。}
\end{mybox}

\begin{mybox}[7. 常用算子 (Operators)]{headerblue}
    \boxdesc{梯度、散度、旋度}
...
\begin{mybox}[8. Python / EinsteinPy 实战]{headerblue}
    \boxdesc{代码工具箱}
    \begin{itemize}
